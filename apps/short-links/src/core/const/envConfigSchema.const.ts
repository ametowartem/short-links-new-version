...
export const JoiSchema: Joi.ObjectSchema = Joi.object({
  ...
  MAIL_MICROSERVICE: Joi.string().required().default('MAILING'),
  MINIO_ACCESS_KEY: Joi.string().required().default('minioadmin'),
  MINIO_SECRET_KEY: Joi.string().required().default('minioadmin'),
  MINIO_HOST: Joi.string().required().default('localhost'),
  MINIO_PORT: Joi.number().required().default(9000),
  MINIO_BUCKET_NAME: Joi.string().required().default('short-links'),
});
